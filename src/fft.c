...
void fft_p2g(fft_t *self, gal_t *catalog, double offset[3])
{
    ...
    for(long p=0; p<catalog->Np; ++p){
        double x = catalog->x[p] * Hinv - offset[0];
        double y = catalog->y[p] * Hinv - offset[1];
        double z = catalog->z[p] * Hinv - offset[2];
        int gi1 = (int)floor(x);
        int gj1 = (int)floor(y);
        int gk1 = (int)floor(z);
        double dx = x - gi1;
    ...
    if(catalog->rand != NULL){
        gal_t *rand = catalog->rand;
        double alpha = (double)catalog->Np / rand->Np;
        ...
        for(long p=0; p<rand->Np; ++p){
            ...
            for(int j=0; j<4; ++j)
            for(int k=0; k<4; ++k)
                F(self, gi[i], gj[j], gk[k]) -= alpha3 * wx[i] * wy[j] * wz[k];
        }
    }
    //double gpp = gsl_pow_3(Ng / self->L) * catalog->V / catalog->Np;
    double H3inv = gsl_pow_3(Ng / self->L);  /* inverse cell volume */
...
void fft_p2k(fft_t *self, gal_t *catalog)
{
    ...
    fft_p2g(self, catalog, offset_dual);
    fft_x2k(self);

    double *f_dual = fft_exportf(self);

    double offset_none[3] = {0, 0, 0};
    fft_p2g(self, catalog, offset_none);
    fft_x2k(self);
